...
6. **Mandatory Code Formatting Rule (dart format):** Before creating or submitting any GitHub Pull Request (PR), AI Agents MUST execute `dart format --set-exit-if-changed .` (or `dart format .`) to format all modified and newly created Dart files. PR creation MUST NOT proceed if unformatted files exist.
